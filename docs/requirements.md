...
- 所有用户可见文案使用稳定文案 ID，并至少提供简体中文和英文资源；服务端错误同时返回稳定 `code`、`message_id` 和按 `Accept-Language` 本地化的 `message`。
- 程序调用必须支持可撤销、可过期、带 Scope 的 PAT；管理面与代理数据面鉴权头必须和目标接口的 `Authorization` 相互隔离。
- 官方 SDK 至少覆盖 Go、Python 和 TypeScript，并提供 CLI；Tvpn Problem 与目标接口自身的非 2xx 响应必须可区分。
...
- 管理员可维护 HTTP 和 SOCKS5 上游代理列表，并分别授权给本地/LDAP 用户和 LDAP 组；用户可选择任一有效授权代理。
- 服务端直连必须支持可选 ACL；未启用 ACL 时兼容现有行为并允许全部用户，启用后只有直接授权或通过 LDAP 组授权的用户可以选择和使用直连，从而支持仅允许指定用户使用代理出口。
